...
{
    use LinkTrait, SpaceTrait, DomainTrait, PixelTrait, UserTrait, UserFeaturesTrait;
    ...
    public function settingsCronjobs(Request $request)
    {
        ...
        return view('admin.content', ['view' => 'admin.settings.cronjobs', 'cronjobs' => $cronjobs]);
    }
    ...
    public function settingsShortener()
    {
        ...
        return view('admin.content', ['view' => 'admin.settings.shortener', 'domains' => $domains]);
    }
    ...
    public function links(Request $request)
    {
        ...
        $links = Link::when($type, function ($query) use ($type) {
            ...
            ->paginate(config('settings.paginate'))
            ->appends(['search' => $search, 'by' => $by, 'sort' => $sort, 'user_id' => $userId]);
    ...
    public function linksEdit($id)
    {
        ...
        return view('admin.content', ['view' => 'links.edit', 'admin' => true, 'domains' => $domains, 'spaces' => $spaces, 'pixels' => $pixels, 'link' => $link, 'features' => $this->getFeatures($user)]);
    }
    ...
    public function domains(Request $request)
    {
        ...
        $domains = Domain::when($search, function ($query) use ($search) {
            ...
            ->orderBy('id', $sort)
            ->paginate(config('settings.paginate'))
            ->appends(['search' => $search, 'sort' => $sort, 'global' => $request->input('global'), 'user_id' => $userId]);
    ...
    public function pixelsEdit(Request $request, $id)
    {
        $pixel = Pixel::where([['id', '=', $id]])->firstOrFail();

        $stats = [
            'links' => Pixel::where('pixel_id', '=', $pixel->id)->count(),
        ];

        return view('admin.content', ['view' => 'pixels.edit', 'admin' => true, 'pixel' => $pixel, 'stats' => $stats]);
    }
    ...
    {
        // The rows to be updated
        $rows = ['short_guest', 'short_bad_words', 'short_protocol', 'short_domain'];
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function updateSettingsCronjobs()
    {
    ...
    public function createPlan(CreatePlanRequest $request)
    {
        ...
        try {
            ...
            $plan->option_pixels = $request->input('option_pixels');
            ...
            $plan->option_targeting = $request->input('option_targeting');
            ...
            $plan->option_data_export = $request->input('option_data_export');
            $plan->save();
        } catch (\Exception $e) {
            $request->flash();
